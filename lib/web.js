...
module.exports = (function () {
    ...
    function WEB() {
        ...
        function getMimeType(type) {
            ...
            for (key in global.config.mime) {
                if (global.config.mime.hasOwnProperty(key)) {
                    if (global.config.mime[key].indexOf(type) > -1) {
                        return key;
                    }
                }
            }
        ...
        function httpListen(port) {
            var server = http.createServer(function (req, res) {
                ...
                if (pathname === '/' || pathname === '/index.html') {
                    readFile(res, 'www/index.html');
                } else {
                    //extension = pathname.match(/\.(.*)$/)[1];
                    readFile(res, 'www' + pathname);
                }
            });
